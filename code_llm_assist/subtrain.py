...
@parameter question_num: which question
"""
def subtrain(train, module, model, criterion, optimizer,  device, test_dataloader, best_loss_dict, epoch_num, question_num):
    k_folds = 5
    ...
    for fold in range(k_folds):
        ...
        # if module is text
        if(module == "text"):
            for train_text, __, train_label, train_sentiments, train_label_num, train_sentiment_num, textname in tqdm(train_dataloader):
                ...
                functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_train_topic.csv",textname, pred1.item(), train_label_num.item(), pooled_output.squeeze().tolist())
                functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_train_sentiment.csv",textname, pred2.tolist(), train_sentiment_num.tolist()[0], pooled_output.squeeze().tolist())
            ...
            with torch.no_grad():
                for val_text, __, val_label, val_sentiments, val_label_num, val_sentiment_num, textname in tqdm(val_dataloader):
                    ...
                    functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_val_topic.csv",textname, pred1.item(), val_label_num.item(), pooled_output.squeeze().tolist())
                    functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_val_sentiment.csv",textname, pred2.tolist(), val_sentiment_num.tolist()[0], pooled_output.squeeze().tolist())
                    
            # testing the model
            with torch.no_grad():
                for test_text, __, test_label, test_sentiments, test_label_num, test_sentiment_num, textname in tqdm(test_dataloader):
                    ...
                    functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_test_topic.csv",textname, pred1.item(), test_label_num.item(), pooled_output.squeeze().tolist())
                    functions.append_to_csv("Epoch_"+str(epoch_num)+"_Fold_"+str(fold)+"_"+module+"_test_sentiment.csv",textname, pred2.tolist(), test_sentiment_num.tolist()[0], pooled_output.squeeze().tolist())
        else:
            for __, train_text, train_label, train_sentiments, train_label_num, train_sentiment_num, textname in tqdm(train_dataloader):
                train_text = train_text[question_num]
                train_label = torch.stack(train_label).squeeze(1).to(device)
                mask = train_text['attention_mask'].to(device) 
                input_id = train_text['input_ids'].squeeze(1).to(device) #squeeze(1) for correspond input, 0 is batch size.
            ...
            # Validating the model
            with torch.no_grad():
                for __, val_text, val_label, val_sentiments, val_label_num, val_sentiment_num, textname in tqdm(val_dataloader):
                    val_text = val_text[question_num]
                    val_label = torch.stack(val_label).squeeze(1).to(device)
                    mask = val_text['attention_mask'].to(device)
                    input_id = val_text['input_ids'].squeeze(1).to(device) #squeeze(1) for correspond input, 0 is batch size.
            ...
            # testing the model
            with torch.no_grad():
                for __, test_text, test_label, test_sentiments, test_label_num, test_sentiment_num, textname in tqdm(test_dataloader):
                    test_text = test_text[question_num]
                    test_label = torch.stack(test_label).squeeze(1).to(device)
                    mask = test_text['attention_mask'].to(device)
                    input_id = test_text['input_ids'].squeeze(1).to(device) #squeeze(1) for correspond input, 0 is batch size.
